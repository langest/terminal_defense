...
CMenu::CMenu()
    : mPlayer(10, 10)
    , mItems({ std::string("Campain"), std::string("Random Level"), std::string("Level Select"), std::string("Exit") })
    , mInputCallbacks() {
...
void CMenu::run() {
    ...
    while ((ch = GUI::getInput()) != 'q') {
        if (ch == 'j' || ch == KEY_DOWN) {
            ...
            (ch == KEY_ENTER || ch == KEY_RIGHT || ch == 'l')
            && mInputCallbacks.find(currentItem) != mInputCallbacks.end()) {
